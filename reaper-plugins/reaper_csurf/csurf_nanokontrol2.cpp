...
** reaper_csurf
**
** Modified by AZSlow3 to work better with Korg nanoKontrol 2
**
** "Marker" < & > buttons are 0x58 and 0x5a 
** 
** rewritten for 7 strips + master
**
**
** MCU support
...
class CSurf_MCU : public IReaperControlSurface
{
    ...
    bool OnFaderMove(MIDI_event_t *evt) {
      if ((evt->midi_message[0]&0xf0) == 0xe0) // volume fader move
      {
        ...
        if (tid>=0&&tid<8 && m_fader_lasttouch[tid]!=0xffffffff)
          m_fader_lasttouch[tid]=m_fader_lastmove;

        if (tid == 7) tid=0; // master offset, master=0
	...
	bool OnRotaryEncoder( MIDI_event_t *evt ) {
	      ...
	      evt->midi_message[1] < 0x18 ) // pan
	  {
	    ...
	    if (tid == 7) tid=0; // adjust for master
	    else tid+=GetBankOffset();
	    MediaTrack *tr=CSurf_TrackFromID(tid,g_csurf_mcpmode);
	    if (tr)
	    {
	...
	bool OnBankChannel( MIDI_event_t *evt ) {
	  ...
	  for (x = 0; x < m_mcu_list.GetSize(); x ++)
	  {
	    ...
	    if (item)
	    {
	      if (item->m_offset+7 > maxfaderpos)
	        maxfaderpos=item->m_offset+7;
	    }
	  }

	  if (evt->midi_message[1]>=0x30) movesize=1;
	  else  movesize=7; // maxfaderpos?
	...
	bool OnRecArm( MIDI_event_t *evt ) {
	  ...
	  if(tid == 7){
	    SendCtrlMIDI(3);
	    return true; // no record arm for master
	  }
	  tid+=GetBankOffset();
	  MediaTrack *tr=CSurf_TrackFromID(tid,g_csurf_mcpmode);
	  if (tr)
	...
	bool OnChannelSelect( MIDI_event_t *evt ) {
	  return true; // nanoKontrol 2 send select on fader moves... 
  ...
  void ClearUndoLed() {
	...
	bool OnReplaceButton( MIDI_event_t *evt ) {
	  SendCtrlMIDI(1);
	  return true;
	}
	...
	bool OnButtonPress( MIDI_event_t *evt ) {
	  ...
	  static const int nPressOnlyHandlers = 21;
	  static const ButtonHandler handlers[nHandlers] = {
	      ...
	      { 0x58, 0x58, &CSurf_MCU::OnReplaceButton,      NULL },
...
public:
    ...
    CSurf_MCU(int offset, int size, int indev, int outdev, int cfgflags, int *errStats) 
    {
      ...
      memset(m_fader_touchstate,0,sizeof(m_fader_touchstate));
      memset(m_fader_lasttouch,0xff,sizeof(m_fader_lasttouch));
      memset(m_pan_lasttouch,0,sizeof(m_pan_lasttouch));
    ...
    const char *GetTypeString() { return "KNK2"; }
    const char *GetDescString()
    {
      m_descspace.SetFormatted(512,
          __LOCALIZE_VERFMT("Korg nanoKontrol 2 (dev %d,%d)","csurf"),m_midi_in_dev,m_midi_out_dev);
...
#define FIXID(id) const int oid=CSurf_TrackToID(trackid,g_csurf_mcpmode); \
    int id=oid; \
    if (id>0) { id -= GetBankOffset(); if (id==7) id=-1; } else if (id==0) id=7;
    ...
    { 
      ...
      {
        if (id < 8) {
          m_midiout->Send(0x90, 0x08+(id&7),solo?0x7f:0,-1); //blink
        } else if (id == 8) {
    ...
    { 
      ...
      for (x = 0; x < m_mcu_list.GetSize(); x ++)
      {
        ...
        {
          if (mcu->m_offset+7 > movesize)
            movesize=mcu->m_offset+7;
        }
      }
...
static IReaperControlSurface *createFunc(const char *type_string, const char *configString, int *errStats)
{
  ...
  return new CSurf_MCU(parms[0],parms[1],parms[2],parms[3],parms[4],errStats);
}
...
reaper_csurf_reg_t csurf_nanokontrol2_reg = 
{
  "KNK2",
